...
public class DownloadMetadataTest {
    ...
    public void testDownloadMetadata() throws InterruptedException, IOException {
        ...
        byte [] infoHash = HexUtil.decodeHex("c1b9e9acef57ff3a673b5a0324d80fc629cd3465");
        byte [] peerId = HexUtil.decodeHex("577b3949f1f1bbe9ebb3a6db3c870c3e9924468f");
        BootstrapBuilder clientFactory = new BootstrapBuilder();
        Bootstrap bootstrap = clientFactory.clone();
        InetSocketAddress inetSocketAddress = new InetSocketAddress("37.21.234.165", 52449);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            executorService.execute(() ->{
                ...
                bootstrap.handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel ch) throws Exception {
                        ChannelPipeline pipeline = ch.pipeline();
                        pipeline.addLast("handler", new MetadataHandler(HexUtil.decodeHex(peerIds.get(finalI)), HexUtil.decodeHex(infoHashes.get(finalI))));
                    }
                });
